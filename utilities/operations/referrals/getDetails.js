...
module.exports = async (appName) => {
  ...
  const indexServerPercent = _.round((1 - result.app_commissions.campaigns_percent)
      * result.app_commissions.index_percent * 100, 5);

  const referralServerPercent = _.round((100 - (result.app_commissions.campaigns_percent * 100)
      - indexServerPercent), 5);

  return {
    result: {
      referralDuration: _.get(rewardsReferral, 'duration', 90),
      campaignServerPercent: result.app_commissions.campaigns_percent * 100,
      indexServerPercent,
